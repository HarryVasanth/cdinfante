...
test('verify docs page features', async ({ page }) => {
  ...
  await page.screenshot({ path: '/home/jules/verification/docs_final_debug.png', fullPage: true });

  // Check for radar chart (PT or EN)
  const radarText = await page.getByText(/Radar Chart|Gráfico de Radar/).isVisible();
